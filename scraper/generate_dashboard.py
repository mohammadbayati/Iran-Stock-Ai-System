...
def build_data(rows, prev_labels):
    """Convert CSV rows to safe JSON-serialisable dicts."""
    out = []
    for r in rows:
...
.kpi:hover{{filter:brightness(1.2)}}
...
th .sort-arrow{{color:#484f58;margin-right:2px}}
...
tr.row.is-stale td{{opacity:.7}}
tr.row.is-conflict td:first-child::after{{content:'⚠️';margin-right:4px;font-size:11px}}
...
.change-icon{{font-size:13px;vertical-align:middle;margin-right:2px}}
.spark{{display:block}}
/* ── Drawer ── */
...
<div class="kpi-bar">
  ...
  <div class="kpi" style="background:#003300;border:1px solid #00c85355;cursor:pointer" onclick="kpiFilter('entry')">
    <div class="kpi-val" style="color:#00c853">{kpi['entry']}</div>
    <div class="kpi-lbl">🟢 کاندید ورود</div>
  </div>
  <div class="kpi" style="background:#1a1400;border:1px solid #ffd74055;cursor:pointer" onclick="kpiFilter('highc')">
    <div class="kpi-val" style="color:#ffd740">{kpi['highc']}</div>
    <div class="kpi-lbl">⭐ High Confidence</div>
  </div>
  <div class="kpi" style="background:#1a0000;border:1px solid #ff525255;cursor:pointer" onclick="kpiFilter('overbought')">
    <div class="kpi-val" style="color:#ff5252">{kpi['overbought']}</div>
    <div class="kpi-lbl">🔴 اشباع خرید</div>
  </div>
  <div class="kpi" style="background:#1a0e00;border:1px solid #ff910055;cursor:pointer" onclick="kpiFilter('conflict')">
    <div class="kpi-val" style="color:#ff9100">{kpi['conflict']}</div>
    <div class="kpi-lbl">⚠️ Risk Conflict</div>
  </div>
  <div class="kpi" style="background:#1c2529;border:1px solid #78909c55;cursor:pointer" onclick="kpiFilter('missing')">
...
<thead>
<tr>
  <th onclick="sortBy('sym',0)"><span class="sort-arrow" id="arr0"></span>نماد</th>
  <th onclick="sortBy('label_fa',1)"><span class="sort-arrow" id="arr1"></span>وضعیت</th>
  <th onclick="sortBy('grade',2)"><span class="sort-arrow" id="arr2"></span>رتبه</th>
  <th onclick="sortBy('score',3)"><span class="sort-arrow" id="arr3"></span>امتیاز</th>
  <th onclick="sortBy('rsi',4)"><span class="sort-arrow" id="arr4"></span>RSI</th>
  <th onclick="sortBy('price',5)"><span class="sort-arrow" id="arr5"></span>قیمت</th>
  <th onclick="sortBy('sector',6)"><span class="sort-arrow" id="arr6"></span>سکتور</th>
  <th onclick="sortBy('vol',7)"><span class="sort-arrow" id="arr7"></span>حجم×</th>
  <th onclick="sortBy('sm',8)"><span class="sort-arrow" id="arr8"></span>پول هوشمند</th>
  <th>نمودار</th>
</tr>
</thead>
...
function kpiFilter(type){{
  ...
  // Reset manual filters when KPI card clicked
...
function sortBy(key,col){{
  ...
  if(arr)arr.textContent=_sortAsc?'↑':'↓';
  render();
}}
...
function render(){{
  ...
  rows.forEach(function(d,i){{
    var cls='row'+(d.missing?' is-missing':'')+(d.stale&&!d.missing?' is-stale':'')+(d.conflict?' is-conflict':'');
    ...
    var changeIcon='';
    if(d.change==='up') changeIcon='<span class="change-icon" title="ارتقاء وضعیت">⬆️</span>';
    else if(d.change==='down') changeIcon='<span class="change-icon" title="افت وضعیت">⬇️</span>';
    else if(d.change==='changed') changeIcon='<span class="change-icon" title="تغییر وضعیت">🔄</span>';
    ...
    var spark=d.close_20d?'<canvas class="spark" id="sp'+i+'" data-p="'+esc(d.close_20d)+'" width="80" height="26"></canvas>':'';
    html+='<tr class="'+cls+'" onclick="openDrawer('+i+')">'
      ...
      +'<td style="text-align:center;color:'+esc(vol>=2?'#00c853':vol>=1?'#ffd740':'#78909c')+'">'+volStr+'</td>'
...
function drawSparklines(){{
  document.querySelectorAll('.spark').forEach(function(c){{
    ...
    p.forEach(function(v,i){{
      var x=i/(n-1)*w,y=h-(v-mn)/rng*(h-4)-2;
...
function renderTopPicks(){{
  ...
  document.getElementById('topPicksRow').innerHTML=picks.map(function(d,i){{
    return '<div class="pick-card" onclick="openDrawerByIdx(DATA.indexOf(d))">'
      ...
      +'<div class="pick-grade" style="color:#8b949e">'+esc(d.rsi_band)+' | '+esc(d.sector)+'</div>'
      +'</div>';
  }}).join('');
  // Fix closure issue
  document.querySelectorAll('.pick-card').forEach(function(card,i){{
    card.onclick=function(){{openDrawer(DATA.indexOf(picks[i]));}}
  }});
}}
...
function openDrawer(idx){{
  ...
  var spark='';
  if(d.close_20d)spark='<canvas id="drawerSpark" data-p="'+esc(d.close_20d)+'" width="340" height="70" style="margin-top:10px;display:block"></canvas>';
  var vol=parseFloat(d.vol)||0;
  function row(label,val){{return val?'<dt>'+label+'</dt><dd>'+esc(val)+'</dd>':''}}
  document.getElementById('drawerContent').innerHTML=
    '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;padding-top:8px">'
    +'<div style="flex:1"><div style="font-size:18px;font-weight:700;color:#e6edf3">'+esc(d.sym)+'</div>'
    +'<div style="margin-top:4px"><span class="badge" style="color:'+esc(d.label_color)+';background:'+esc(d.label_bg)+'">'+esc(d.label_fa)+'</span>'
    +' <span style="color:'+esc(d.grade_color)+';font-weight:700;font-size:15px">'+esc(d.grade)+'</span>'
    +' <span style="color:#8b949e;font-size:13px">امتیاز '+d.score.toFixed(0)+'</span></div>'
    +change+'</div></div>'
    +conflict+missing+stale
    +'<div class="drawer-section"><h4>دلایل تصمیم</h4><p style="color:#ccc;font-size:12px;line-height:1.7">'+esc(d.reasons)+'</p></div>'
    +'<div class="drawer-section"><h4>عوامل امتیازدهی</h4><p style="color:#8b949e;font-size:11px;line-height:1.7">'+esc(d.factors)+'</p></div>'
  ...
  if(d.close_20d){{
    setTimeout(function(){{
      var c=document.getElementById('drawerSpark');
      if(!c)return;
      var p=c.dataset.p.split(',').map(Number).filter(function(n){{return n>0;}});
      if(p.length<2)return;
      var ctx=c.getContext('2d'),w=c.width,h=c.height,n=p.length;
      ...
      p.forEach(function(v,i){{
        var x=i/(n-1)*w,y=h-(v-mn)/rng*(h-6)-3;
        i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);
      }});
      ctx.stroke();
    }},50);
...
function exportCSV(){{
  ...
  filtered().forEach(function(d){{
    rows.push([d.sym,d.label_fa,d.grade,d.score.toFixed(0),d.rsi,d.rsi_band,d.price,d.sector,
      parseFloat(d.vol)>0?parseFloat(d.vol).toFixed(2)+'x':'',d.sm,d.q,d.reasons]);
  }});
...
def run():
    ...
    with open(DASHBOARD_PATH, "w", encoding="utf-8") as f:
        f.write(page)
    print(f"[dashboard] Phase-1 Pro → {DASHBOARD_PATH} | {len(rows)} symbols | KPI: {kpi}")
